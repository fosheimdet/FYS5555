...
public:
  ...
  MC_Bayes(Int_t NchanA, Double_t intLumA, Double_t intLumSigmaA, Double_t* backgroundA, Double_t* backgroundSigmaA, 
	   Int_t* NobsA, Int_t NmcA, Double_t* signalEffA, Double_t* signalEffSigmaA, Bool_t doRefPriorA, Bool_t doLogNormalEffA,
	   Bool_t doLogNormalLumiA, Bool_t doLogNormalBkgA, Int_t logNormalMode = 2, Bool_t doGammaEffA = false, Int_t gammaMode = 1,
	   Double_t signalEffGaussTruncA = 0.0, Bool_t doModGaussEffA = false, Double_t modGaussPowerA = 1.0) {
    
    intLum = intLumA;
    intLumSigma = intLumSigmaA;
    background = backgroundA;
    backgroundSigma = backgroundSigmaA;
    Nobs = NobsA;
    Nmc = NmcA;
    Nchan = NchanA;
    ...
    logNormalSigmaBkg = new Double_t[Nchan];
	...
	exit(1);
      ...
      if (logNormalMode == 1) //mean
	logNormalMuBkg[chan] = TMath::Log(background[chan]/TMath::Sqrt(1.0 + TMath::Power(backgroundSigma[chan]/background[chan],2.0)));
      else if (logNormalMode == 2) //median
	logNormalMuBkg[chan] = TMath::Log(background[chan]);
      else if (logNormalMode == 3) //mode
	logNormalMuBkg[chan] = TMath::Log(background[chan]*(1.0 + TMath::Power(backgroundSigma[chan]/background[chan],2.0)));
      else {
	cout << "Unrecognized log-normal mode\n";
	exit(1);
      }

      logNormalSigmaBkg[chan] = TMath::Sqrt(TMath::Log(1.0 + TMath::Power(backgroundSigma[chan]/background[chan],2.0)));
    }
  ...
  Double_t CalcLikelihood(Double_t signal) {
    ...
    Double_t thisIntLum,thisBackground,s,b,lambda,thisSignalEff;
    Double_t sum = 0.0;
	...
	if (backgroundSigma[chan] == 0.0)
	  thisBackground = background[chan];
	else {
	  if (doLogNormalBkg) 
	    thisBackground = TMath::Exp(rnd->Gaus(logNormalMuBkg[chan],logNormalSigmaBkg[chan]));
	  else {
	    do
	      thisBackground = rnd->Gaus(background[chan],backgroundSigma[chan]);
	...
	if (signalEffSigma[chan] == 0.0)
	  ...
	  if (doLogNormalEff) {
	    ...
	    while (thisSignalEff < signalEffGaussTrunc || thisSignalEff > 1.0);
	  }
	}
  ...
  Double_t* expectedExclusionSingleChan(Double_t h = 0.001, Int_t Nmc2 = 5000, Bool_t verbose = true) {
    ...
    //Monte Carlo loop
    Int_t thisNobs;
    Double_t thisIntLum,thisBackground;
  ...
  Double_t* expectedExclusion(Double_t h = 0.001, Int_t Nmc2 = 5000, Bool_t verbose = true, Bool_t needAvgAndStd = false) {
    ...
    Bool_t calced;
    Double_t thisIntLum,thisBackground,exclusion;
    ...
    Double_t* returnValue = new Double_t[6];
    //returnValue[2] = sum/Nmc2;
...
//     vector<Int_t> cumuExcl;
//     cumuExcl.clear();
//     vector<Int_t> cumuExcl2;
//     cumuExcl2.clear();
//     for (UInt_t i = 0; i < exclusionsI.size(); i++) {
//       cumuExcl.push_back(0);
//       cumuExcl2.push_back(0);
//       for (UInt_t j = 0; j <= i; j++)
// 	cumuExcl[i] += exclusionsI[j];
//       for (UInt_t j = i; j < exclusionsI.size(); j++)
// 	cumuExcl2[i] += exclusionsI[j];
//     }
    
//     //Set 1 and 2 sigma boundaries
//     for (UInt_t i = 0; i < cumuExcl.size()-1; i++) {
      
//       if (cumuExcl[i]*1.0/Nmc2 <= twoSigmaProb && cumuExcl[i+1]*1.0/Nmc2 > twoSigmaProb) {
// 	returnValue[0] = exclusionVals[i];
// 	if (verbose) std::cout << "Lower 2sigma tail = " << cumuExcl[i]*1.0/Nmc2 << std::endl;
//       }

//       if (cumuExcl[i]*1.0/Nmc2 <= oneSigmaProb && cumuExcl[i+1]*1.0/Nmc2 > oneSigmaProb) {
// 	returnValue[1] = exclusionVals[i];
// 	if (verbose) std::cout << "Lower 1sigma tail = " << cumuExcl[i]*1.0/Nmc2 << std::endl;
//       }
//     }

//     for (UInt_t i = cumuExcl2.size()-1; i > 0; i--) {

//       if (cumuExcl2[i]*1.0/Nmc2 <= oneSigmaProb && cumuExcl2[i-1]*1.0/Nmc2 > oneSigmaProb) {
// 	returnValue[3] = exclusionVals[i];
// 	if (verbose) std::cout << "Upper 1sigma tail = " << cumuExcl2[i]*1.0/Nmc2 << std::endl;
//       }
      
//       if (cumuExcl2[i]*1.0/Nmc2 <= twoSigmaProb && cumuExcl2[i-1]*1.0/Nmc2 > twoSigmaProb) {
// 	returnValue[4] = exclusionVals[i];
// 	if (verbose) std::cout << "Upper 2sigma tail = " << cumuExcl2[i]*1.0/Nmc2 << std::endl;
//       }
//     }

//     //Handling of low background region
//     if (cumuExcl[0]*1.0/Nmc2 > oneSigmaProb)
//       returnValue[1] = exclusionVals[0];
//     if (cumuExcl[0]*1.0/Nmc2 > twoSigmaProb)
//       returnValue[0] = exclusionVals[0];

//     if (cumuExcl2[cumuExcl2.size()-1]*1.0/Nmc2 > oneSigmaProb)
//       returnValue[3] = returnValue[2];
//     if (cumuExcl2[cumuExcl2.size()-1]*1.0/Nmc2 > twoSigmaProb)
//       returnValue[4] = returnValue[3];
